...
from diffusers.models.unet_2d_condition import get_up_block as get_up_block_2d
from .unet_blocks_2d import get_down_block as get_down_block_2d
...
from .unet_blocks_3d import get_up_block as get_up_block_3d
from .unet_blocks_3d import get_down_block as get_down_block_3d
...
class UNet2DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(self,
        ...
        sample = rearrange(sample, 'b c t h w -> (b t) c h w')  # for t dimension
        # print(f"sample shape: {sample.shape}")
        ...
        if encoder_hidden_states is not None and encoder_hidden_states.shape[0] != sample.shape[0]:
        ...
        for idx, downsample_block in enumerate(self.down_blocks):
            ...
            if train_or_sample == 'train':
                ...
                skeleton_feature = features_adapter[idx]
                # print(f"skeleton_feature shape: {skeleton_feature.shape}")
                skeleton_feature = rearrange(skeleton_feature, 'b c t h w -> (b t) c h w')  # for t dimension
                # print(f"skeleton_feature shape: {skeleton_feature.shape}")
        ...
        sample = rearrange(sample, '(b t) c h w -> b c t h w', t=video_length)  # for t dimension
